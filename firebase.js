...
function normalizeAdjustments(adjustments = {}) {
  return Object.entries(adjustments || {})
    .map(([id, adjustment]) => ({
      id,
      player: String(adjustment?.player || "").trim(),
      value: Number(adjustment?.value || 0),
      reason: String(adjustment?.reason || "").trim(),
      timestamp: Number(adjustment?.timestamp || 0),
    }))
    .filter((adjustment) => adjustment.player && Number.isFinite(adjustment.value))
    .sort((first, second) => second.timestamp - first.timestamp);
}

function applyAdjustments(players, adjustments) {
  adjustments.forEach((adjustment) => {
    if (!players[adjustment.player]) {
      players[adjustment.player] = normalizeLedgerEntry(null);
    }

    players[adjustment.player].total += adjustment.value;
    players[adjustment.player].available =
      players[adjustment.player].total - players[adjustment.player].locked;
  });
...
function hydrateRoomData(room) {
  ...
  const adjustments = normalizeAdjustments(room?.adjustments || {});
  const players = applyAdjustments(
    normalizePlayersMap(room?.players || {}, playersList),
    adjustments
  );
...
export async function addAdjustment(roomCode, { player, value, reason = "" }) {
  const normalizedRoomCode = normalizeRoomCode(roomCode);
  const normalizedPlayer = String(player || "").trim();
  const normalizedValue = Number(value);

  if (!normalizedRoomCode || !normalizedPlayer || !Number.isFinite(normalizedValue)) {
    throw new Error("Dati correzione non validi.");
  }

  const database = ensureDatabase();
  const adjustmentId = `adj_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;

  await set(ref(database, `rooms/${normalizedRoomCode}/adjustments/${adjustmentId}`), {
